...
- `scripts/analysis/concept_rotation_report.py`: 从本地 `stock_daily_latest` 计算概念扩散度、RRG、龙头确认和次日条件候选，输出 JSON/CSV/PDF；概念成分来自新浪财经并带本地缓存。
- `scripts/ops/run_concept_rotation_report.sh`: 从服务器 `.env` 安全读取 `WEB_HOOK`，在 `quantmind` 容器内生成日报并推送飞书摘要和 PDF 下载链接。
- `scripts/ops/install_concept_rotation_cron.sh`: 幂等安装每天 20:00 的概念轮动日报宿主机 cron 任务。
