...
class QlibBacktestRequest(BaseModel):
    ...
    def parse_strategy_params(cls, data: Any) -> Any:
        if isinstance(data, dict):
            strat_type = str(data.get("strategy_type", "TopkDropout")).lower()
            params = data.get("strategy_params", {})
            if strat_type == "sector_momentum_leader_core" and isinstance(params, dict):
    ...
    strategy_params: SectorMomentumLeaderCoreParams | QlibStrategyParams = Field(
